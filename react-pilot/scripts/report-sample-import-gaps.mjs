 ...
 *   node scripts/report-sample-import-gaps.mjs --dir public/demo-records
 *   node scripts/report-sample-import-gaps.mjs --dir fixtures/mission --dir ../MANTA\ End\ User\ Testing/samples
 */
...
function main() {
  const { dirs: dirArgs } = parseSampleDirs(process.argv.slice(2))
  let all = []
  if (dirArgs.length) {
    for (const d of dirArgs) all.push(...collectXml(d))
  } else {
  ...
  if (!all.length) {
    console.error(
      'No XML files found. Pass valid paths with --dir (relative to react-pilot/), e.g. --dir public/demo-records',
    )
    process.exit(1)
  }
  ...
  if (dirArgs.length) {
    lines.push('')
    lines.push(`**Scoped directories:** ${dirArgs.map((d) => `\`${d}\``).join(', ')} (${all.length} files)`)
  }
  lines.push('')
