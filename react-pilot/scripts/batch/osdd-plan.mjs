/**
 * OSDD / OpenSearch link patch planner.
 *
 * Takes a CSV of record UUIDs + OSDD descriptor URLs, loads matching XML,
 * generates per-record insertion previews, and optionally applies them.
 *
 * CSV format (headers required):
 *   uuid, osddUrl, label, description
 *
 * Usage:
 *   node scripts/batch/osdd-plan.mjs --csv osdd.csv --xml-dir input/ --out reports/osdd-plan
 *   node scripts/batch/osdd-plan.mjs --csv osdd.csv --xml-dir input/ --apply
 *
 * Outputs (preview mode, default):
 *   <out>/osdd-plan.json
 *   <out>/osdd-plan-diff.csv
 *
 * @module scripts/batch/osdd-plan
 */
...
function parseArgs(argv) {
  const out = {
    csvPath: '',
    xmlDir: '',
    outDir: path.join(REACT_PILOT, 'reports/osdd-plan'),
    apply: false,
  }
...
function buildOsddSnippet(row) {
  const url = (row.osddUrl || row.url || '').trim()
  const label = (row.label || 'OpenSearch Description Document').trim()
  const desc = (row.description || row.desc || 'OpenSearch Description Document (OSDD) for granule discovery').trim()
    ...
    <gmd:MD_DigitalTransferOptions>
      <gmd:onLine>
        <gmd:CI_OnlineResource>
          ...
          <gmd:function>
            <gmd:CI_OnLineFunctionCode codeList="http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#CI_OnLineFunctionCode" codeListValue="search">search</gmd:CI_OnLineFunctionCode>
          </gmd:function>
        </gmd:CI_OnlineResource>
      </gmd:onLine>
    </gmd:MD_DigitalTransferOptions>
...
async function main() {
  ...
  if (!opts.csvPath || !fs.existsSync(opts.csvPath)) {
    console.error('Provide --csv <path> with columns: uuid, osddUrl, label, description')
    process.exit(1)
  }
  ...
  console.log(`Loaded ${csvRows.length} OSDD rows from CSV`)
  ...
  for (const row of csvRows) {
    ...
    const osddUrl = (row.osddUrl || row.url || '').trim()

    if (!uuid || !osddUrl) {
      console.warn(`  SKIP row: missing uuid or osddUrl`)
      continue
    }

    if (!isOsddUrl(osddUrl)) {
      console.warn(`  WARN ${uuid}: URL does not look like OSDD — ${osddUrl}`)
    }

    const rec = xmlByUuid.get(uuid)
    if (!rec) {
      plan.push({ uuid, status: 'NOT_FOUND', osddUrl, snippet: null })
      diffRows.push({ uuid, status: 'NOT_FOUND', osddUrl, xmlFile: '' })
      continue
    }

    const snippet = buildOsddSnippet(row)
  ...
  writeJson(path.join(opts.outDir, 'osdd-plan.json'), summary)
  writeCsv(path.join(opts.outDir, 'osdd-plan-diff.csv'), diffRows, ['uuid', 'status', 'osddUrl', 'xmlFile'])
